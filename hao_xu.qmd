...
```
According to the picture, the module 1 has an average grade around 12, the varience is smaller to module one which means the grades are more concentrated. In module 2, the average grade is 10, and it has a bigger varience which means the students may have more differentiated grades.
...
```
According to the picture, *Cryptography and Codebreaking* and *Airship Piloting and Navigation* shows a positive relationship.
## Question21
```{r}
...
avg_grades_per_course |>
  ...
  geom_point(size = 3)
```
The grades of *Cryptography and Codebreaking* and *Etiquette and Social Graces* are correlated positively.
...
ggplot(final_grades_df, aes(x = group, y = final_grade)) +
  geom_boxplot()
```
The picture shows that the final grades of groups are quite different. The group 5 has a relatively high final grade distribution than other groups, while the group 18 has a relatively low final grade. Most of the groups have a median grade close to 11. 
...
ggplot(final_grades_df, aes(x = group, y = final_grade, fill = sex)) +
  geom_boxplot()
```
According to the picture, most of the groups do not show significant differences in final grades regarding to the sex, while several groups(13, 2, 3, 7, 15, 16) have very significant differences. In most of the case, female students show a relatively low final grades (except for the group 15).

## Question26
```{r}
cond1_course_check <- student_course_averages |>
  group_by(id) |>
  summarise(
    pass_condition_1 = min(course_avg) >= 5,
    .groups = "drop"
  )

module_averages <- student_course_averages |>
  group_by(id, module) |>
  summarise(
    mod_avg = mean(course_avg), 
    .groups = "drop"
  )

cond2_module_check <- module_averages |>
  group_by(id) |>
  summarise(
    pass_condition_2 = all(mod_avg >= 10),
    .groups = "drop"
  )

pass_fail_df <- final_grades_df |>
  left_join(cond1_course_check, by = "id") |>
  left_join(cond2_module_check, by = "id") |>
  mutate(pass = pass_condition_1 & pass_condition_2) |>
...
```
## Question27
The number of students who failed but have a final grade >= 10 is `r pass_fail_df |> filter(pass == FALSE, final_grade >= 10) |> nrow()`.

## Question28
```{r}
pass_rate_per_group <- pass_fail_df |>
  group_by(group) |>
  summarise(pass_rate = mean(pass))
ggplot(pass_rate_per_group, aes(x = group, y = pass_rate)) +
  geom_col()
```
Group 1, 5, 4, 9 have the highest pass rate(100%), while group 18 has the lowest pass rate(54%).

## Question29
```{r}
failing_courses <- student_course_averages |>
  inner_join(pass_fail_df, by = c("id", "group", "sex")) |>
  filter(pass == FALSE, course_avg < 5) |>
  count(course, name = "failure_count") |>
  arrange(desc(failure_count))
failing_courses |>
  slice_head(n = 5) |>
  knitr::kable()
```
Electrical Engineering and Telegraphy is the course the most responsible of student failure.

## Question30
```{r}
age_success_analysis <- pass_fail_df |>
  inner_join(students, by = c("id", "group", "sex")) |>
...
ggplot(age_success_analysis, aes(x = age, y = success_rate)) +
  geom_line() +
  geom_point()
```
There are some ages that shows a high pass rate, but the sample size is too small to get a concret conclusion.
